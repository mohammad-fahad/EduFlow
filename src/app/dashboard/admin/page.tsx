...
export default async function AdminPage() {
  ...
  return (
    <div className="space-y-6 page-enter">
      <div>
        <h1 className="font-display text-[22px] font-bold tracking-tight text-slate-800">প্রধান শিক্ষক ড্যাশবোর্ড</h1>
        <p className="text-[13px] text-slate-400 mt-1">
          প্রতিষ্ঠানের সব কার্যক্রম পরিচালনা করুন
          {session.isDevOverride && <span className="ml-2 text-[10px] font-bold bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">DEV MODE</span>}
        </p>
      </div>
      <StatGrid />
      <DevInfo session={session} />
    </div>
  );
}
...
// ════════════════════════════════════════════════════════════
// (copy this file, change the function name + role check + title)
// import { getResolvedSession } from "@/lib/get-role-server";
// import { redirect } from "next/navigation";
...
// ════════════════════════════════════════════════════════════
// Shared sub-components (used by all role pages)
// ════════════════════════════════════════════════════════════
function StatGrid() {
  return (
...
function DevInfo({ session }: { session: { isDevOverride: boolean; role: string; institutionName: string | null } }) {
  if (!session.isDevOverride) return null;
  return (
